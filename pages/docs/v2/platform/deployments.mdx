...
export const meta = {
  ...
  lastEdited: '2019-12-11T10:40:03.000Z'
}
...
There are two types of logs available, **Build Time** and **Runtime**.
...
**Runtime** logs are generated by [Serverless Functions](/docs/v2/serverless-functions/introduction) whilst the application is running. **Runtime** logs are stored in memory only as they arrive from the Serverless Function and are **not persisted**.

The only exception to this is **failed requests**. If a request fails, the log for this will be stored indefinitely whereas all other **Runtime** logs will be lost when navigating away from the page.

<Note type="hint">
  If you wish to persist <strong>Runtime</strong> logs, you can use the{' '}
  <Link href="/docs/api#endpoints/log-drains">Log Drains API</Link> to store
  them with a third-party service.
</Note>
