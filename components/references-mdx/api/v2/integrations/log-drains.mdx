...
When you choose the `ndjson` type, the URL receives a HTTPS or HTTP POST request with JSON objects delimited by new line (`\\n`) on the `POST` body.
...
For example, if you have a basic HTTP server subscribing to Log Drains, payload can be validated like the following:
